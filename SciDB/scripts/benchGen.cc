 ...
 * v1.4: fix bug in per-attribute output.
 */
...
std::string Tiles::getModeName(Mode mode){
 	switch (mode){
	    case CSV:
		return ".CSV";
	    case BINARY:
...
int main(int argc, char** argv) {
    ...
    std::string format = tiles.getModeName(outputMode);
    std::cout << "Generating a dataset of " << config << " size in " << format << " format..." << std::endl;
    ...
    std::cout << "Dataset of size " << config << " has been successfully generated in " << format << " format." << std::endl;
    return 0;
}
